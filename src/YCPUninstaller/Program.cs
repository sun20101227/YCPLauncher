    ...
    internal static class Program
    {
        ...
        static void Main()
        {
            ...
            try
            {
                ...
                string launcherPath = Path.Combine(currentDir, "YCPLauncher.exe");
                if (!string.Equals(Path.GetFileName(currentDir), "YCPLauncher", StringComparison.OrdinalIgnoreCase) ||
                    !File.Exists(launcherPath))
                {
                    throw new InvalidOperationException(
                        "卸载程序不在有效的 YCPLauncher 安装目录中，已拒绝执行目录删除。");
                }

                string cmd = $"/d /c timeout /t 2 /nobreak > nul & rd /s /q \"{currentDir}\"";
