...
public class RevealBorder : Border
{
    ...
    private readonly Stopwatch _pointerThrottle = Stopwatch.StartNew();
    ...
    {
        if (!IsMouseOver || _hoverBrush.Opacity <= 0)
            return;

        Point mousePos = e.GetPosition(this);
        if (_pointerThrottle.ElapsedMilliseconds < 16 &&
            Math.Abs(mousePos.X - _lastPointerPosition.X) < 1.5 &&
            Math.Abs(mousePos.Y - _lastPointerPosition.Y) < 1.5)
            return;

        _pointerThrottle.Restart();
    ...
    {
        if (Services.ConfigService.AreAnimationsEnabled && HoverColor.A > 0)
        {
    ...
    {
        if (_hoverBrush.Opacity <= 0 && _bgHoverBrush.Opacity <= 0)
            return;
