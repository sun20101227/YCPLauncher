...
public class ApiService
{
    ...
    public ApiService()
    {
        ...
        {
            Timeout = TimeSpan.FromSeconds(Math.Clamp(cfg.ApiTimeout, 3, 120))
        };
    }
        ...
        try
        {
            ...
            foreach (var asset in assets.EnumerateArray())
            {
                ...
                if (selectedAsset == null ||
                    name.Contains("YCPInstaller", StringComparison.OrdinalIgnoreCase) ||
                    name.Contains("Setup", StringComparison.OrdinalIgnoreCase))
                {
                    selectedAsset = asset;
                }

                if (name.Contains("YCPInstaller", StringComparison.OrdinalIgnoreCase))
                    break;
            }

            if (selectedAsset.HasValue)
            {
                downloadUrl = selectedAsset.Value.GetProperty("browser_download_url").GetString() ?? "";
                fileSize = selectedAsset.Value.GetProperty("size").GetInt64();
            }
